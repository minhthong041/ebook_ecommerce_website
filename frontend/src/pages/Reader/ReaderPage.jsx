...
const FONTS = [
  { id: "serif", name: "Có chân (Serif)", family: "Georgia, 'Times New Roman', serif" },
  { id: "sans-serif", name: "Không chân (Sans-Serif)", family: "'Inter', -apple-system, sans-serif" },
];
...
export default function ReaderPage() {
  ...
  if (!book) {
    return (
      <div className="reader-error">
        ...
        <Link to="/library" className="btn btn-primary">Quay lại Thư viện</Link>
      </div>
    );
  }
  ...
  useEffect(() => {
    localStorage.setItem(`reader_annotations_${bookId}`, JSON.stringify(annotations));
  }, [annotations, bookId]);

  useEffect(() => {
    localStorage.setItem(`reader_bookmarks_${bookId}`, JSON.stringify(bookmarks));
  }, [bookmarks, bookId]);
  ...
  const handleChapterChange = (index) => {
    ...
    localStorage.setItem(`reader_progress_${bookId}`, JSON.stringify({
      chapterIndex: index,
  ...
  const activeChapter = book.tableOfContents[activeChapterIndex] || book.tableOfContents[0];
      ...
      try {
        ...
        if (pNode && pNode.hasAttribute("data-index")) {
          ...
          setSelectionData({
            ...
            coords: {
              top: rect.top - contentRect.top + contentRef.current.scrollTop - 48,
  ...
  const toggleBookmark = () => {
    if (isCurrentChapterBookmarked) {
      setBookmarks((prev) => prev.filter((b) => b.chapterIndex !== activeChapterIndex));
    } else {
      const newBookmark = {
        id: Date.now(),
        chapterIndex: activeChapterIndex,
  ...
  const renderParagraph = (text, paragraphIndex) => {
    const paragraphAnnotations = annotations.filter(
      (ann) => ann.chapterIndex === activeChapterIndex && ann.paragraphIndex === paragraphIndex
    );

    if (paragraphAnnotations.length === 0) {
      return (
        <p key={paragraphIndex} data-index={paragraphIndex} className="reader-paragraph">
          {text}
        </p>
      );
    }
  ...
  const activeThemeClass = THEMES.find((t) => t.id === preferences.theme)?.class || "reader-theme--sepia";
  const activeFontFamily = FONTS.find((f) => f.id === preferences.fontFamily)?.family || "serif";
  const activeFontSizeValue = FONT_SIZES.find((s) => s.id === preferences.fontSize)?.size || "1.1rem";
    ...
    <div className={`reader-container ${activeThemeClass}`}>
      ...
      <header className="reader-header">
        <div className="reader-header__left">
          <Link to="/library" className="reader-back-btn" title="Quay lại thư viện">
            ‹ Thư viện
          </Link>
        ...
        <div className="reader-header__center">
          <span className="reader-header__chapter-title">{activeChapter.title}</span>
        </div>
        <div className="reader-header__right">
          <button
            ...
            title={isCurrentChapterBookmarked ? "Bỏ đánh dấu chương này" : "Đánh dấu chương này"}
          >
            🔖
          </button>
          <button
      ...
      <div className="reader-body">
        {/* SIDEBAR */}
        <aside className={`reader-sidebar ${sidebarOpen ? "reader-sidebar--open" : ""}`}>
          ...
          <div className="reader-sidebar__content">
                  ...
                  {book.tableOfContents.map((chap, idx) => (
                    <li key={idx} className="reader-toc__item">
                      ...
                      >
                        ...
                        <span className="reader-toc__title-text">{chap.title}</span>
                      </button>
                    </li>
                  ))}
                </ul>
            ...
            {activeTab === "annotations" && (
              <div className="reader-annotations">
                <div className="reader-annotations__section">
                  <h3 className="reader-sidebar__title">Đã đánh dấu chương ({bookmarks.length})</h3>
                  {bookmarks.length === 0 ? (
                    <p className="reader-sidebar__empty-text">Chưa có chương nào được đánh dấu.</p>
                  ) : (
                    <ul className="reader-bookmarks__list">
                      {bookmarks.map((b) => (
                        <li key={b.id} className="reader-bookmarks__item">
                          ...
                          >
                            <span>📌 Chapter {b.chapterIndex + 1}: {b.chapterTitle}</span>
                            <span className="reader-bookmarks__date">{b.createdAt}</span>
                          </button>
                        </li>
                      ))}
                    </ul>
                ...
                <div className="reader-annotations__section">
                  <h3 className="reader-sidebar__title">Highlights & Ghi chú ({annotations.length})</h3>
                  {annotations.length === 0 ? (
                    <p className="reader-sidebar__empty-text">Bôi đen văn bản để highlight hoặc thêm ghi chú.</p>
                  ) : (
                      ...
                      {annotations.map((ann) => (
                        ...
                        >
                          <div
                            ...
                            onClick={() => handleChapterChange(ann.chapterIndex)}
                            title="Đi tới chương chứa ghi chú này"
                          >
                            "{ann.selectedText}"
                          </div>
                          {ann.note && <div className="reader-notes__comment">💬 {ann.note}</div>}
                          <div className="reader-notes__footer">
                            <span className="reader-notes__location">Chương {ann.chapterIndex + 1}</span>
                            <button
            ...
            {activeTab === "settings" && (
              <div className="reader-settings">
                <h3 className="reader-sidebar__title">Tùy biến không gian đọc</h3>
                
                {/* Theme Selector */}
                <div className="reader-settings__group">
                  <label className="reader-settings__label">Màu nền trang sách</label>
                ...
                <div className="reader-settings__group">
                  ...
                  <div className="reader-settings__font-list">
                    {FONTS.map((font) => (
                      <button
                        key={font.id}
                        onClick={() => setPreferences((prev) => ({ ...prev, fontFamily: font.id }))}
                ...
                {/* Font Size Selector */}
                <div className="reader-settings__group">
                  <label className="reader-settings__label">Cỡ chữ hiển thị</label>
        ...
        >
          ...
          >
            <div className="reader-content-body">
              <span className="reader-chapter-indicator">Chương {activeChapterIndex + 1}</span>
              <h1 className="reader-chapter-title-main">{activeChapter.title}</h1>
              <div className="reader-chapter-separator" />
              
              <div className="reader-paragraphs-container">
                {chapterParagraphs.map((text, idx) => renderParagraph(text, idx))}
              </div>
              ...
              <div className="reader-page-nav">
                ...
                <span className="reader-nav-progress">
                  Chương {activeChapterIndex + 1} / {book.tableOfContents.length}
                </span>
                <button
                  onClick={() => handleChapterChange(activeChapterIndex + 1)}
                  disabled={activeChapterIndex === book.tableOfContents.length - 1}
            ...
            >
              {!showNoteForm ? (
                <div className="reader-tooltip__actions">
                  ...
                  <button onClick={clearSelection} className="reader-tooltip__close-btn">
                    ✕
                  </button>
                </div>
              ) : (
                <form onSubmit={handleAddNote} className="reader-note-form">
                  <div className="reader-note-form__colors">
                    <span style={{ fontSize: "0.8rem", color: "var(--text-secondary)" }}>Màu nhãn:</span>
      ...
      {viewingAnnotation && (
        <div className="reader-modal-overlay" onClick={() => setViewingAnnotation(null)}>
          <div className="reader-modal-card" onClick={(e) => e.stopPropagation()}>
            <div className="reader-modal-header">
              <h3>Chi tiết Annotation</h3>
              <button className="reader-modal-close" onClick={() => setViewingAnnotation(null)}>
                ✕
              </button>
            </div>
            <div className="reader-modal-body">
              <p className="reader-modal__label">Đoạn văn bản trích dẫn:</p>
              <blockquote className={`reader-modal__quote reader-modal__quote--${viewingAnnotation.color}`}>
                "{viewingAnnotation.selectedText}"
              </blockquote>
              
              <p className="reader-modal__label" style={{ marginTop: "16px" }}>Ghi chú của bạn:</p>
              {viewingAnnotation.note ? (
                <p className="reader-modal__note-text">{viewingAnnotation.note}</p>
              ) : (
                <p className="reader-modal__note-empty">Chưa có ghi chú nào được lưu cho đoạn văn này.</p>
              )}
              <p className="reader-modal__meta">Được tạo ngày: {viewingAnnotation.createdAt} - Chương {viewingAnnotation.chapterIndex + 1}</p>
            </div>
            <div className="reader-modal-footer">
              ...
              <button onClick={() => setViewingAnnotation(null)} className="btn btn-primary">
                Đóng
              </button>
            </div>
          </div>
